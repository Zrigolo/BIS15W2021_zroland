...
penguins %>% 
  filter(sex!= "NA") %>% 
  group_by(sex) %>% 
  summarise(avg_bill_lgth=mean(bill_length_mm))
```
...
penguins %>% 
  ...
  group_by(sex) %>% 
  summarise(avg_bill_lgth=mean(bill_length_mm)) %>% 
  ggplot(aes(x=sex, y=avg_bill_lgth)) +
  geom_col()
```
...
```{r}
homerange %>% 
  select(log10.mass, family, trophic.guild, class) %>% 
  filter(trophic.guild== "carnivore", class== "mammalia") %>% 
  ggplot(aes(x=reorder(family,log10.mass), y=log10.mass))+geom_col()+coord_flip()
```
